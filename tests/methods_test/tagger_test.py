...
def test_taginfo_xref_plus() -> None:
    assert (
        Tagger.taginfo(3, 'INDI', 'hello', xref='@1@') == '3 @1@ INDI hello\n'
    )


def test_taginfo_xref_plus_with_atsign() -> None:
    assert (
        Tagger.taginfo(3, 'INDI', '@hello', xref='@1@')
        == '3 @1@ INDI @@hello\n'
    )
...
def test_taginfo_xref_full() -> None:
    assert (
        Tagger.taginfo(3, 'INDI', '@hello', 'extra', xref='@1@')
        == '3 @1@ INDI @@hello extra\n'
    )
...
def test_string_payload_multiline() -> None:
    assert (
        Tagger.string('previous\n', 3, 'NOTE', 'This\nis\na note.')
        == 'previous\n3 NOTE This\n4 CONT is\n4 CONT a note.\n'
    )


def test_string_payload_multiline_list() -> None:
    assert (
        Tagger.string('previous\n', 3, 'NOTE', ['This\nis\na note.'])
        == 'previous\n3 NOTE This\n4 CONT is\n4 CONT a note.\n'
    )
...
def test_string_xref_plus() -> None:
    assert (
        Tagger.string('', 3, 'INDI', 'hello', xref='@1@')
        == '3 @1@ INDI hello\n'
    )
...
def test_string_xref_full() -> None:
    assert (
        Tagger.string('', 3, 'INDI', '@hello', 'extra', xref='@1@')
        == '3 @1@ INDI @@hello extra\n'
    )
...
def test_string_with_list() -> None:
    assert (
        Tagger.string('', 3, 'WWW', ['https://here.com', 'https://there.com'])
